    ...
    <div className="absolute z-50 left-0 top-full mt-2 bg-white p-4 shadow-lg rounded-lg">
      <div className="w-auto py-[1.5rem] px-[1.6rem]">
        <ul className="space-y-6">
          {/* Kiểm tra nếu groupedCategories không phải là null và có dữ liệu */}
          {groupedCategories && Object.keys(groupedCategories).length > 0 ? (
            Object.keys(groupedCategories).map((categoryDadId) => {
              ...
              const categoryName = categoryDadNames[parseInt(categoryDadId)] || "Mục cha khác"; // Lấy tên mục cha từ ánh xạ
            ...
            <p className="text-center text-gray-500">Không có danh mục nào</p> // Hiển thị khi không có dữ liệu
          )}
        </ul>
      </div>
    </div>
