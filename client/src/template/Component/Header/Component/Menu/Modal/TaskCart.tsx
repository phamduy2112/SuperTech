...
function TaskCart({ onClose }: TaskCart) {
  ...
  return (
    <div
      ...
      onClick={onClose} // Đóng TaskCart khi nhấp vào backdrop
    >
      <div
        className="w-[350px] h-[100vh] bg-[#E5E5E5]"
        onClick={stopPropagation} // Ngăn chặn sự kiện click lan ra ngoài
      >
        <div className="w-[100%] py-[2rem] px-[1rem] flex justify-between items-center">
          <div className="text-[1.8rem] font-semibold">GIỎ HÀNG</div>
            ...
            <div className="bg-[white] px-2 py-[1rem]">
              {listCart.map((item) => (
                <div key={item.product_id} className="flex py-[1rem] border border-b-[gray]">
                  ...
                  <div
                    className="text-[2rem] cursor-pointer text-[#7500CF]"
              ...
              <div className="flex justify-end pt-6">
                <h4 className="text-[1.5rem] font-semibold">
                  Tổng đơn: <span>{formatCurrencyVND(totalPrice)}đ</span>
                </h4>
              </div>
            </div>
            <div className="w-[100%] bg-[#7500CF] py-[1rem] px-[1rem] cursor-pointer">
              <div
                className="text-[1.5rem] text-white text-center"
